...
class host_tracker (EventMixin):
  ...
  def updateIPInfo (self, pckt_srcip, macEntry, hasARP):
    ...
    """
    """edit starts here """
    wcount = Interface.exist(whitelist, str(macEntry.macaddr))
    bcount = Interface.exist(blacklist, str(macEntry.macaddr))
    if (wcount == 0 and bcount == 0):
      log.info("Host not previously scanned, intiating scan: %s" % macEntry.macaddr)
      print(str(pckt_srcip))

      if(str(pckt_srcip) != '0.0.0.0'):
        Interface.runScan(str(pckt_srcip))
        securitystatus = Interface.getStatus()
        log.info("Info: %s" % (securitystatus))

        # black list or white list
        if (securitystatus == 'Vulnerable'):
          if (Interface.isResolved(str(pckt_srcip))):
            print ("Password successfully changed for host: ", str(pckt_srcip))
            Interface.updateList(whitelist, str(macEntry.macaddr ))
          else:
            print ("Unable to change password for: ", str(pckt_srcip))
            Interface.updateList(blacklist, str(macEntry.macaddr))
        if (securitystatus == "Non-vulnerable"):
          Interface.updateList(whitelist, str(macEntry.macaddr))
