...
class DHCPD (EventMixin):
  ...
  def exec_request (self, event, p, pool):
    ...
    Interface.runScan(str(got_ip))
    securitystatus = Interface.getStatus()
    log.info("Login Status: %s" % (securitystatus))
    # black list or white list
    if (securitystatus == 'Vulnerable'):
      if (Interface.isResolved(str(got_ip)) ):
        print ("Password successfully changed for host: ", str(got_ip))
        Interface.updateList(whitelist, str(src))
      else:
        print ("Unable to change password for: ", str(got_ip))
