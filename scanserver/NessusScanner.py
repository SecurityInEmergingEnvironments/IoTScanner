...
Connetion is done via a REST -API
"""
...
def download(sid, fid, token):
    ...
    print('Saving scan results to {0}/{1}.'.format(resultpath, filename))
    path = "{0}/{1}".format(resultpath, filename)
...
def runScan(ipaddress):
    ...
    update(scan_id, scan_data['name'], scan_data['description'], ipaddress, token)
